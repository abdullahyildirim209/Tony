...
class FeatureEngine:
    """Builds 38-dim observation vectors from streaming daily bars.
    ...
    def __init__(
        ...
        self.turbulence_lookback = turbulence_lookback

        # Rolling buffer of daily bars: {date, open, high, low, close, volume, fng}
        self.bars: list[dict] = []
        # Need enough history for turbulence_lookback + sma_long + window_size + padding
        self.buffer_size = max(sma_long, turbulence_lookback) + window_size + 10
    ...
    def _compute_turbulence(self) -> float:
        """Compute turbulence as z-score of rolling volatility."""
        closes = [b["close"] for b in self.bars]
        if len(closes) < self.turbulence_lookback + 1:
            return 0.0

        # Compute daily returns for all available history
        returns = []
        ...
        if len(returns) < self.turbulence_lookback:
            return 0.0

        # Rolling volatility (last turbulence_lookback days)
        current_vol = np.std(returns[-self.turbulence_lookback:])

        # Expanding mean/std of rolling vol (approximate with all available history)
        # Compute rolling vol for all windows
        n = len(returns)
        lb = self.turbulence_lookback
        if n < lb + 1:
            return 0.0

        rolling_vols = []
        for i in range(lb, n + 1):
            rolling_vols.append(np.std(returns[i - lb:i]))
        rolling_vols = np.array(rolling_vols)

        vol_mean = np.mean(rolling_vols)
        vol_std = np.std(rolling_vols)
        if vol_std > 0:
            return float((current_vol - vol_mean) / vol_std)
        return 0.0

    def get_obs(self, position_state: dict) -> np.ndarray | None:
        """Build 38-dim observation vector.

        Args:
            position_state: {is_long: bool, entry_price: float, current_price: float}

        Returns:
            np.ndarray of shape (38,) or None if insufficient history.
        """
        ...
        turb = self._clip_scalar(turb, "turbulence")

        obs = np.concatenate([
            pct_changes,
            [sma_ratio, rsi_norm, fng_norm, bp, turb, flat, long, unrealized_pnl],
        ]).astype(np.float32)
    ...
    def load_clip_stats(train_npz_path: str) -> dict:
        ...
        stats = {}
        for col in ["pct_changes", "sma_ratios", "rsi_norm", "fng_norm", "buy_pressure", "turbulence"]:
